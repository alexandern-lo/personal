...
namespace Avend.ApiTests.ControllerTests.DashboardController
{
    ...
    public class Dashboard_GetSuperadminAvgEventsHistoryMonthly_TypeIsAll : BaseDashboardEndpointTest
    {
        public const string DashboardUrl = "dashboard/superadmin/history/average_events/monthly?type=all";
        ...
        public async Task ShouldReturnProperLeadsHistoryWhenPreparedForLastMonthAnd2MonthsAgo()
        {
            ...
            var lastMonthEventDto = await bobEventsData.AddFromSample();
            bobEventsData.UpdateDbRecordCreationTime(lastMonthEventDto.Uid.GetValueOrDefault(), DateTime.UtcNow.AddMonths(-1));
            var earlierEventDto = await bobEventsData.AddFromSample();
            bobEventsData.UpdateDbRecordCreationTime(earlierEventDto.Uid.GetValueOrDefault(), DateTime.UtcNow.AddMonths(-2));
