...
namespace Avend.API.Infrastructure.Validation
{
    public static class CountryValidations
    {
        public static string[] CountriesList = 
        {
            ...
            "Romania", "Russia", "Rwanda", "Saint Barthélemy", "Saint Helena", "Saint Kitts and Nevis", "Saint Lucia",
            "Saint Martin", "Saint Pierre and Miquelon", "Saint Vincent", "Samoa", "San Marino", "São Tomé and Príncipe",
            "Saudi Arabia", "Senegal", "Serbia", "Seychelles", "Sierra Leone", "Singapore", "Sint Maarten", "Slovakia",
            "Slovenia", "Solomon Islands", "Somalia", "South Africa", "South Georgia", "South Korea", "Spain",
            "Sri Lanka", "Sudan", "Suriname", "Svalbard and Jan Mayen", "Sweden", "Swaziland", "Switzerland", "Syria",
        ...
        public static Check<string> IsValidCountry(this Check<string> check, string message = "{Key} should be a valid country name", Action<Error> onError = null)
        {
            if (!CountriesList.Contains(check.Value))
            {
                var error = new Error(ErrorResponse.ErrorMessageInvalidParameter)
                {
                    {
                        AvendValidations.ErrorCode,
                        ErrorCodes.CodeInvalidParameter
                    }
                };
