...
namespace Recurly.AspNetCore
{
    public class Invoice : RecurlyEntity
    {
        ...
        /// <returns></returns>
        public Task<byte[]> GetPdf(string acceptLanguage = "en-US")
        {
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<CouponRedemption> GetRedemption()
        {
            var redemptionList = await GetRedemptions();
            return redemptionList.HasAny() ? redemptionList[0] : null;
        }

        public async Task<RecurlyList<CouponRedemption>> GetRedemptions()
        {
        ...
        {
            return await Invoices.Get(OriginalInvoiceNumberWithPrefix());
        }
        ...
        public async Task<Invoice> Refund(Adjustment adjustment, bool prorate = false, int quantity = 0, RefundOrderPriority refundPriority = RefundOrderPriority.Credit)
        {
            var adjustments = new List<Adjustment>();
            adjustments.Add(adjustment);

            return await Refund(adjustments, prorate, quantity, refundPriority);
        }

        public async Task<Invoice> Refund(IEnumerable<Adjustment> adjustments, bool prorate = false, int quantity = 0, RefundOrderPriority refundPriority = RefundOrderPriority.Credit)
        {
            var refunds = new RefundList(adjustments, prorate, quantity, refundPriority);
            var invoice = new Invoice();

            var response = await Client.Instance.PerformRequest(Client.HttpRequestMethod.Post,
                memberUrl() + "/refund",
                refunds.WriteXml,
                invoice.ReadXml);
        ...
        public async Task<Invoice> RefundAmount(int amountInCents, RefundOrderPriority refundPriority = RefundOrderPriority.Credit)
        {
    ...
    public sealed class Invoices
    {
        ...
        /// <returns></returns>
        public static async Task<Invoice> Get(int invoiceNumber)
        {
            return await Get(Convert.ToString(invoiceNumber));
        }

        /// <summary>
        /// Look up an Invoice.
        /// </summary>
        /// <param name="invoiceNumberWithPrefix">Invoice Number</param>
        /// <returns></returns>
        public static async Task<Invoice> Get(string invoiceNumberWithPrefix)
        {
            var invoice = new Invoice();
        ...
        /// <returns></returns>
        public static async Task<Invoice> Create(string accountCode)
        {
            var invoice = new Invoice();
