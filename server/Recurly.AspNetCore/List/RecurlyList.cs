...
using HttpRequestMethod = Recurly.AspNetCore.Client.HttpRequestMethod;
...
namespace Recurly.AspNetCore.List
{
    public abstract class RecurlyList<T> : IEnumerable<T> where T : RecurlyEntity
    {
        ...
        public Task RetrievalTask { get; private set; }
        ...
        public int Capacity => capacity < 0 ? Count : capacity;
        ...
        internal RecurlyList(HttpRequestMethod method, string url)
        {
            ...
            RetrievalTask = GetItems();
        }

        public async Task GetItems()
        {
            await Client.Instance.PerformRequest(Method,
                ApplyPaging(BaseUrl),
                ReadXmlList);
        }
