...
namespace Recurly.AspNetCore
{
    public class Plan : RecurlyEntity
    {
        ...
        public RecurlyList<AddOn> AddOns
        {
            get
            {
                if (addOns == null)
                {
                    ...
                    addOns = new AddOnList(url);
                }
                return addOns;
            }
        }

        private Dictionary<string, int> unitAmountInCents;
        /// <summary>
        /// A dictionary of currencies and values for the subscription amount
        /// </summary>
        public Dictionary<string, int> UnitAmountInCents
        {
            get { return unitAmountInCents ?? (unitAmountInCents = new Dictionary<string, int>()); }
        }

        private Dictionary<string, int> setupFeeInCents;
        /// <summary>
        /// A dictionary of currency and values for the setup fee
        /// </summary>
        public Dictionary<string, int> SetupFeeInCents => setupFeeInCents ?? (setupFeeInCents = new Dictionary<string, int>());
        ...
        public async Task<object> GetAddOn(string addOnCode)
        {
            var addOn = new AddOn();

            var status = await Client.Instance.PerformRequest(Client.HttpRequestMethod.Get,
    ...
    public sealed class Plans
    {
        ...
        /// <returns></returns>
        public static async Task<Plan> Get(string planCode)
        {
            var plan = new Plan();
