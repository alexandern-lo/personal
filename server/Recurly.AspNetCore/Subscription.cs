...
namespace Recurly.AspNetCore
{
    ...
    public class Subscription : RecurlyEntity
    {
        ...
        {
            get { return address ?? (address = new Address()); }
            set { address = value; }
        }
        ...
        public Account Account => account ?? (account = Task.Run(async () => await Accounts.Get(accountCode)).Result);
        ...
        {
            get { return invoice ?? (invoice = Task.Run(async () => await Invoices.Get(invoiceNumber)).Result); }
        }

        private Plan plan;
        public Plan Plan
        {
            get { return plan ?? (plan = Task.Run(async () => await Plans.Get(PlanCode)).Result); }
            set
            {
                plan = value;
        ...
        {
            get { return coupon ?? (coupon = Task.Run(async () => await AspNetCore.Coupons.Get(couponCode)).Result); }
            set
            {
                coupon = value;
        ...
        public Coupon[] Coupons
        {
            get {
                if (coupons == null)
                {
                    coupons = new Coupon[couponCodes.Length];
                }
                ...
                if (coupons.Length == 0)
                {
                    ...
                    for (int i = 0; i<couponCodes.Length; i++)
                    {
                        ...
                        coupons[i] = Task.Run(async () => await AspNetCore.Coupons.Get(code)).Result;
                    }
                }
            ...
            set {
                coupons = value;
                couponCodes = new string[coupons.Length];
                for (int i = 0; i<coupons.Length; i++)
                {
                    couponCodes[i] = coupons[i].CouponCode;
                }
            }
        }
        ...
        public void Preview(ChangeTimeframe timeframe)
        {
            if (_saved)
            {
                throw new RecurlyException("Cannot preview an existing subscription.");
            }
        ...
        /// </summary>
        /// <param name="timeframe">ChangeTimeframe.Now (default) or at Renewal</param>
        public virtual async Task<Subscription> PreviewChange(ChangeTimeframe timeframe)
        {
            if (!_saved)
            {
                throw new RecurlyException("Must have an existing subscription to preview changes.");
            }
        ...
        public virtual async Task<Subscription> PreviewChange()
        {
            return await PreviewChange(ChangeTimeframe.Now);
        }
                ...
                switch (reader.Name)
                {
                    ...
                    case "invoice":
                        ...
                        if (!href.IsNullOrEmpty())
                            invoiceNumber = Uri.UnescapeDataString(href.Substring(href.LastIndexOf("/") + 1));
                        else
        ...
        protected void WriteSubscriptionXml(XmlWriter xmlWriter)
        {
            ...
            if (couponCodes != null && couponCodes.Length != 0) {
                ...
                foreach (var _coupon_code in couponCodes)
                {
    ...
    public sealed class Subscriptions
    {
        ...
        public static async Task<Subscription> Get(string uuid)
        {
